 ...
 * @method
 */
class Co extends User_Controller
{
			...
			$result['hits'][$key]['shipping'] = $this->addFreeDirectMail($shippings, $value, $rates, $weights, $bs, $ue);
		        ...
		        $result['hits'][$key]['shipping'] = [];
				    ...
				    if ($ship = $this->getFeeTwo($y, $value, $rates, $weights, $ems)) {
    				    ...
    				    break;
    			    	case 3:
				    if ($ship = $this->getFeeThree($y, $value, $rates, $weights, $ue)) {
                                    	$result['hits'][$key]['shipping'][] = $ship;
                                    }
    				    break;
    			    	case 4:
				    if ($ship = $this->getFeeFour($y, $value, $rates, $weights, $bs)) {
                                    	$result['hits'][$key]['shipping'][] = $ship;
                                    }
    				    break;
			    	case 5:
		       ...
		       if (is_array($result['hits'][$key]['shipping']) && count($result['hits'][$key]['shipping'])) {
			   ...
			   if (in_array(1, $luxuries) && !in_array(4, $shipping_types)) {
			       if ($ship = $this->addLuxuryShipping($shippings, $value, $rates, $weights, $bs, $ue)) {
				   $result['hits'][$key]['shipping'][] = $ship;
			       }
			   }
	...
	foreach ($options as $k => $v) {
		...
		if ((($v['upper'] == 0) && ($value['price'] >= ($v['lower'] * $rate))) || (($value['price'] >= ($v['lower'] * $rate)) && ($value['price'] < ($v['upper'] * $rate)))) {
		    if ($type == 'discount') {
			$cut = ceil($value['price'] * (1 - $v['promotion']));
			$ty = 2;
		    } elseif ($type == 'money') {
			$ty = 1;
			$cut = ceil($v['promotion'] * $rate);
		    } elseif ($type == 'LDF') {
			$cut = 0;
			$ty = 5;
		    } elseif ($type == 'DM') {
   ...
   private function groupByField($pds, $products, $field = 'store')
   {
	...
	foreach ($products as $k => $v) {
		...
		if ($v['id'] == $y['id']) {
			...
			if ($field == 'store') {
			    $st[$merge[$field]]['store_name'] = $v['store_name'];
	...
	if (isset($count['shipping_fee'])) {
	    $count['shipping_fee'] = ceil($count['shipping_fee']);
   ...
   private function getStoreFee($y, $store, $rate, $weights)
   {
	...
	if ($y['count_type'] == 3) {
            ...
            if ((($y['high'] == 0) && ($store['price'] > ($rate*$y['low']))) || (($y['high'] != 0) && ($store['price'] >= ($rate*$y['low'])) && ($store['price'] < ($rate*$y['high'])))) {
                $store_fee  = $rate*$y['low_fee'];
            }
        }
