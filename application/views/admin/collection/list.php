...
$this->layout->placeholder('title', '专辑管理');
?>
    ...
    <script>
        $(function () {
            $("#SelectAll").click(function () {
                ...
                var obj = $("#collection-list-ctner").find("input");
                for(var i=0;i<obj.length;i++)
                {
                    obj[i].checked = type;
                }
                $("#SelectAll").attr("checked", type);
            });
        });
    </script>
    <div class="m-content">
            ...
            <div class="col-lg-12" style=" padding-bottom:30px">
                <section class="box ">
                    <header class="panel_header">
                        <h2 class="title pull-left"><input type="checkbox" value="全选" id="SelectAll"/>全选</h2>
                    </header>
...
<li>
    <input type="checkbox" name="collection_ids" value="{{:id}}" class="subcheck"/>
    <div>
        <a href="/collection/detailOld?collection_id={{:id}}&rt=mobile" target="_blank"><img src="{{if cover_image}}{{:cover_image}}@140h_1wh{{else}}data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D{{/if}}" alt="{{:title}}" class="img-responsive" data-toggle="tooltip" title="点击预览" width="100%"/></a>
        <div class="jieshao" style="">[ID:{{:id}}]{{:title}}</div>
