...
class CloudsearchClient
{
    ...
    private function _curl($url, $params = array(), $method = self::METHOD)
    {
        ...
        $options = array(CURLOPT_HTTP_VERSION => 'CURL_HTTP_VERSION_1_1', CURLOPT_CONNECTTIMEOUT => $this->connect_timeout, CURLOPT_TIMEOUT => $this->timeout, CURLOPT_CUSTOMREQUEST => $method, CURLOPT_HEADER => false, CURLOPT_RETURNTRANSFER => true, CURLOPT_USERAGENT => "opensearch/php sdk " . $this->sdkVersion,//php sdk 版本信息
    ...
    private function _socket($url, $params = array(), $method = self::METHOD)
    {
        ...
        $content = $this->_buildRequestContent($parse, $method, http_build_query($params, '', '&'));
        if ($this->debug) {
            $this->debugInfo = $content;
        }

        $socket = fsockopen($parse['host'], $parse["port"], $errno, $errstr, $this->connect_timeout);
    ...
    private function _parseResponse($response)
    {
        ...
        $ret = array();
        $ret["result"] = (isset($header['Content-Encoding']) && trim($header['Content-Encoding']) == 'gzip') ? $this->_gzdecode($response, $header) : $this->_checkChunk($response, $header);
        $ret["info"]["http_code"] = isset($header["http_code"]) ? $header["http_code"] : 0;
