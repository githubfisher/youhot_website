...
class Base64 {
	...
	private $_PADCHAR = '=';
	private function _alpha_gender($index =0){
		$alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

		$len =  (1<<(($index>>2) +1));//只取2/4/8

		if($len >0 ){
			$arr = str_split($alpha, $len);
			$this->_alpha = implode('', array_reverse($arr));
		}else{
			$this->_alpha = $alpha;
		}
	...
	public function encode($s='',$index = 0) {

		if($index> 0){
			$this->_alpha_gender($index);
		}
		$s = (string)$s;
	    $x = array();
  	...
  	public function decode ($s='', $index =0){
